...
func main() {
	...
	cmd.IntOption("retaindays", "store", "<days>", "Activate retention and deduplicate daily backups older than 24h. Limit to one per day for <days> (default 0 = keep all daily)", &retainDays, cmd.Standard|cmd.Preference)
	cmd.IntOption("retainweeks", "store", "<weeks>", "Activate retention and deduplicate daily backups older than 24h. Limit to one per week for <weeks> (default 0 = keep all weekly)", &retainWeeks, cmd.Standard|cmd.Preference)
	cmd.BoolOption("retainyearly", "store", "Activate retention and deduplicate daily backups older than 24h. Keep the last backup of each year", &retainYearly, cmd.Standard|cmd.Preference)
